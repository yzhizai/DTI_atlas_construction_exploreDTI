function atlas_con_demo
% DT = DT_recon;
% DWI_con(DT, 1);


diffeoFile = spm_select(1, 'image', 'choose a diffeo field file');
[pat, tit, ext, ~] = spm_fileparts(diffeoFile);
filename = cellstr(fullfile(pat, [tit, ext]));
comp = {struct('def', {filename})};
[Def,~] = get_comp(comp);

DTIFile = spm_select(1, 'mat', 'choose a ExploreDTI dataset');
DT = load(DTIFile);
DT = DTIResample(DT.DT, Def);
