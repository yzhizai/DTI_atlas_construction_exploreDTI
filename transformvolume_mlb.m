...
u_index_array = zeros(Isize(1), 1);
i_array = zeros(Isize(1), 1);
v_index_array = zeros(Isize(2), 1);
j_array = zeros(Isize(2), 1);
w_index_array = zeros(Isize(3), 1);
k_array = zeros(Isize(3), 1);

for x = 1:Isize(1)
    u_index_array(x) = mod(x, dx)*4 + 1;
    i_array(x) = floor(x/dx);
end
for y = 1:Isize(2)
    v_index_array(y) = mod(y, dy)*4 + 1;
    j_array(y) = floor(y/dy);
end
for z = 1:Isize(3)
    w_index_array(z) = mod(z, dz)*4 + 1;
    k_array(z) = floor(z/dz);
end
...
%             Tlocalz = 0;
%             for l = 1:4
%                 if (i + l) > 0 && (i + l) <= Osize(1)
%                     for m = 1:4
%                         if (j + m) > 0 && (j + m) <= Osize(2)
%                             for n = 1:4
%                                 if (k + n) > 0 && (k + n) <= Osize(3)
%                                      indexO=(i+l)+(j+m)*Osize(1)+(k+n)*Osize(1)*Osize(2);
%                                      val=Bu(l+u_index)*Bv(m+v_index)*Bw(n+w_index);
%                                      Tlocalx=val*Ox(indexO) + Tlocalx;
%                                      Tlocaly=val*Oy(indexO) + Tlocaly; 
%                                      Tlocalz=val*Oz(indexO) + Tlocalz;
%                                 end
%                             end
%                         end
%                     end
%                 end
%             end
%             indexI=x+y*Isize(1)+z*Isize(1)*Isize(2);
% 
%             Iout(indexI)=interpolate_3d_cubic_black(Tlocalx, Tlocaly, Tlocalz, Isize, Iin);
%         end
%     end
% end
